...
EFRport = r'COM7'
#OVport = r'COM16'
header0 = "OV OFF Time,OV OFF GGvolt,OV OFF GGcurrent,Vsysmin,OV OFF Vsysavg, OV OFF IspkrmaX, OV OFF IspkrAVG, IRLEDS SIREN Time,IRLEDS SIREN GGvolt,IRLEDS SIREN GGcurrent,IRLEDS SIREN Vsysmin,IRLEDS SIREN Vsysavg,IRLEDS SIREN IspkrmaX,IRLEDS SIREN IspkrAVG, \n"
...
#EFR.setRTS(True)
LOG0 = r'C:\Users\EIzazaga\OneDrive - Arlo Technologies, Inc\Desktop\Aquila Stuff\SpeakerDroopTest\SpeakerDroop_testCode\TestLogs\ClippingDroopTest_Ver3_Real_Alkaline_2.txt'
...
#        global END_CLP
    ...
    while(MIN_CLP != -27):
            ...
            if(CLP_LVL > END_CLP):
                ...
                writeToLog(LOG0, GGvolt, GGcur)
                time.sleep(2)
            elif(CLP_LVL < MIN_CLP):
                print("in second comparison")
                #print("")
                print("CLP_LVL = " + str(CLP_LVL) + " " + "MIN_CLP = " + str(MIN_CLP) )
                GGvolt, GGcur = getGGinfo()
                ...
                if(VsysMinGlobal < 3.3 or VsysAvgGlobal < 3.5):
                    print("Vsys Min is: " + str(VsysMinGlobal))
                    print("Vsys Avg is: " + str(VsysAvgGlobal))
                    MIN_CLP = CLP_LVL - 3
                    print("MIN_CLP: " + str(MIN_CLP + 3) +  " is now: " + str(MIN_CLP))
                    time.sleep(2)
                    #time.sleep(10)
                    #update globals 
                    ttModuleSendISP('pwm_on 0 0')
